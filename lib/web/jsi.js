...
var Log = {
    ...
    trace : function(){},
    test : function(){},
    info : console.warn.bind(console.window,'INFO: '),
    ...
    conf: function(typ, on) {
        ...
        if (!typ) typ = 'debug';
        switch (typ) {
            case 'debug': LogDebug = Log.debug = (on?console.error.bind(console.window,'DEBUG: '):function(){}); break;
            case 'trace': LogTrace = Log.trace = (on?console.warn.bind(console.window,'TRACE: '):function(){}); break;
            case 'test': LogTest = Log.test = (on?console.error.bind(console.window,'TEST: '):function(){}); break;
...
var jsi = {
            ...
            if (tt == 'undefined')
                ...
                switch (i) {
                    ...
                    case 'insert':
                        ...
                        break;
                    case 'interp':
                        if (vv.logOpts.Debug) Log.conf('debug', true);
                        if (vv.logOpts.Trace) Log.conf('trace', true);
                        if (vv.logOpts.Test)  Log.conf('test', true);
                        break;
                }
    ...
    websock:function(opts) { // Create websocket connection.
        var ws;
        ...
        self.wsstarted = 1;
        var w = {prot:'ws', onmessage:onmessage};
        if (opts) {
            for (var i in opts) {
                if (w[opts[i]]===undefined)
                    throw('expected: '+Object.keys(w));
                w[i] = opts[i];
            }
        }
    ...
    matchObj:function(msg, match, partial, noerror) { //msg:object,match:string=void,partial=false,noerror=false
        function validate(msg, match, partial) { //msg:object,match:string=void,partial=false
            ...
            if (pat === match)
                return;
            if (!partial)
                throw('matchOjb failed: expected "'+pat+'" not "'+match+'"');
    ...
    _main: function(f) { // Startup.
        ...
        if (jsi.config.onload)
            jsi.config.onload();
        var scr = $('script[src$="/jsi.js?websock=true')[0];
        if (scr)
            jsi.websock();
    }
};
