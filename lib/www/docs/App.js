...
if (sess && !isfossil) {
    var ssp = sess.split('.'); // Following not used right now.
...
var appData = {
    ...
    s_showAllSub:false,
    s_navFns:[],
    s_project:{ label:'JSI', title:"home page for jsish", href:"https://github.com/pcmacdon/jsish" },
};

function uencode(str) {
    if (histmode) return str;
    ...
    methods: {
        ...
        goTo:function(nam) {
            if (this.s_mdFile !== nam)
                this.$router.push(Tpath+uencode(nam));
        },
        menuCmd:function(op){
            dputs("OP", op);
            switch (op) {
        ...
        subChange:function() {
            this.s_showAllSub = !this.s_showAllSub;
        }
    },
    computed:  {
...
Vue.component('jsi-left-menu-item', {
    ...
    methods: {
        ...
        selSMC:function() {  // Select item from left menu (MD document menu)
            ...
            var nrt = Tpath+uencode(this.mitem.value);
...
Vue.component('jsi-right-menu-item', {
    ...
    methods: {
        ...
        selTM:function() { // Select item from right menu (TOC)
            ...
            var nrt = Tpath+uencode(this.s_mdFile+'#'+this.mitem.anchor);
            if (this.$router.currentRoute.path !== nrt) {
                this.s_pickedAnc = this.mitem.idx;
                this.s_curAnc = this.mitem.idx;
    ...
    methods: {
        ...
        HrefToRouterPush:function() {
            ...
            for (var i=0; i<l.length; i++) {
                ...
                vr = uencode(vr);
                l[i].onclick=this.routePushFn(Tpath+vr);
            }
        },
        routePushFn:function(vr) {
            var r = this.$router;
            return function() {
                r.push(vr);
                return false;
        ...
        setupAncChain:function(c) { // Update s_ancChain on scroll.
            ...
            this.s_curAnc = c;
            this.s_curAncParent= ((ta && ta.parent)?ta.parent.idx:-1);
        ...
        loadIndexFile:function(fn) {
            ...
            Jsish.ajax({url:url,
                success:function(data, okstr, req) {
                    ...
                    if (!that.s_server)
                        that.s_server = req.getResponseHeader('SERVER');
                },
                error:function(e) {
        ...
        loadFinishIdx:function(done) {
            ...
            if (src.project)
                this.s_project = src.project;
            if (src.navFns)
                this.s_navFns = src.navFns;
            for (var f in svals) {
                var sv = svals[f];
                for (var i = 0; i<sv.length; i+=2) {
                    var ss = sv[i+1];
        ...
        loadRightMenu:function(mdIn) {
            ...
            if (this.s_mdFile)
                return;
            this.s_mdFile = mdIn[0];
            this.s_dirty = true;
            var nrt = Tpath+uencode(mdIn[0]);
            if (this.$router.currentRoute.path !== nrt)
                this.$router.push(nrt);
        },
...
var ws = Jsish.websock({
    ...
    onrecv:function(msg) {
        ...
        switch (msg.cmd) {
            case 'start':
                dputs('STARTED');
                appData.s_websock = ws;
                break;
            case 'exit':
                 dputs("exit done");
                 appData.s_websock = null;
                 break;
        }
    },
});
