  ...
  <b-row rows="1" class="bg-navbar bd-navbar">
    ...
    methods: 
       Object.assign(
        ...
        loadMdFile() {
            let url = this.s_mdDir+this.s_mdFile, that = this;
            dputs("FETCH", url);
            url += '?mimetype=text/markdown';
            fetch(url).
                then(function(response) { that.fetchStat = response.status; return response.text()} ).
                then(function(result) { that.src = result; }).
                then(function(p) { that.loadFinish(true); });
        },
        loadFinish(doHi) {
        ...
        handleScroll(ev) {
            ...
            let current = sections.length - 
                sections.reverse()
                .findIndex(function(section) { window.scrollY >= section.offsetTop - sectionMargin; } ) - 1;
        ...
        loadIndexDir() {
            let url = this.s_mdDir + '?callback=null', that = this;
            fetch(url).
                then(function(response) { that.fetchStatDir = response.status; return response.text() }).
                then(function(result) { that.srcDir = result; }).
                then(function(p) { that.loadFinishDir(true) });
        },
        loadFinishDir(done) {
        ...
        loadIndexFile(fn) {
            ...
            fetch(url).
                then(function(response) { 
                    that.fetchStatIdx = response.status;
                    if (!that.s_server)
                        that.update_s_server(response.headers.get('SERVER'));
                    return response.text() }).
                then(function(result) { that.srcIdx = result; }).
                then(function(p) { that.loadFinishIdx(true)});
        },
        loadFinishIdx(done) {
...
let isfossil = Jsish.isfossil();
let histmode = Jsish.config.histmode;
let mdDir = (histmode ? './md/':'../md/');

// Following not used right now.
let sess = Jsish.getCookie('sessionJsi'), iflags = 0, uflags = 0;
if (sess && !isfossil) {
    var ssp = sess.split('.');
...
Jsish.css('app.css');
    ...
    document.getElementById('app').innerText= 'ERROR: '+JSON.stringify(e);
}
